...
/**
 * WhatsApp is the real B2B channel in Egypt — well ahead of email at this
 * ticket size. Server component, no JS shipped.
 */
export function WhatsAppFab({ locale, message }: { locale: Locale; message?: string }) {
  const t = getDict(locale)
  return (
    <a
      href={waLink(message ?? t.fab.message)}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={t.fab.label}
      className="fixed bottom-5 z-50 flex items-center gap-2 rounded-full bg-[#25D366] px-4 py-3 font-bold text-white shadow-[var(--e-shadow-lg)]"
    ...
    >
      ...
      <span className="hidden sm:inline">{t.fab.text}</span>
    </a>
  )
}
